...
const { removeRecurr, removeEmpties } = require('@/utils/obj-ops');
...
export async function getAllPosts() {
        ...
        folder.posts = await Promise.all(
          folder.posts
            ?.map(async (post) => {
              ...
              post.matter.orig = '';
              // @NOTE: must return null here, otherwise undefined will be added to the array
              // and undefined cannot be serialized when passing values to nextjs page as props
              if (post?.matter?.data?.status !== 'published') return null;
  ...
  // ensure no empty folders make it to the client Side
  // folders = folders?.filter((folder) => folder?.posts[0]);
